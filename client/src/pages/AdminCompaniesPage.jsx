...
export function AdminCompaniesPage() {
  ...
  function handleDelete(company) {
    showModal({
      ...
      message: `Excluir permanentemente "${company.nome}"? Todos os dados vinculados (clientes, vendedores, status) serao removidos.`,
      actions: [{
        label: 'Excluir',
        variant: 'danger',
        onClick: async () => {
            ...
            <div key={company.id} className="rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden">
              <div className={`flex items-center gap-3 px-4 py-3 transition ${!company.ativo ? 'opacity-60' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-500">{company.user_count ?? 0} usuario(s)</p>
                </div>
                  ...
                  {!companyUsers[company.id] ? (
                    ...
                    <>
                      {companyUsers[company.id].length === 0 ? (
                        <p className="text-xs text-zinc-500 mb-3">Nenhum usuario vinculado a esta empresa.</p>
                      ) : (
